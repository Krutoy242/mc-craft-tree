...
export function parseRawRecipes(groups, parsedData) {
  ...
  function pushNodeFnc(raw, isForced) {
    ...
    const found = graph.nodes.find(n =>
      isForced ? node.match(n) : n.match(node)
    )
  ...
  graph.nodes.sort(function(a, b) {
    return (
      a.inputs.length - b.inputs.length || b.outputs.length - a.outputs.length
    )
  })
